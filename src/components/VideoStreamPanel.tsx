...
export default function VideoStreamPanel({
  ...
  // Fetch available camera/video sources
  useEffect(() => {
    fetch(`${API_BASE}/api/vlm/stream/videos`)
      ...
      .then((data) => {
        if (data.ok && data.videos) {
          ...
          // Auto-select phone camera if connected
          const phoneCam = data.videos.find((v: AvailableVideo) => v.path === 'cam:1');
          if (phoneCam && (source === 'webcam' || !source)) {
            setSource('cam:1');
          }
        }
      })
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] text-gray-400 uppercase tracking-widest font-mono flex items-center gap-1.5">
...
function EventRow({ event, index }: { event: StreamEvent; index: number }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 py-2 px-3 rounded-lg text-[11px] transition-all border ${isFrame
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex rounded-lg border border-white/10 overflow-hidden text-[11px] font-semibold">
                  <button
                    ...
                    className={`px-3 py-1 flex items-center gap-1.5 transition-colors ${playerMode === 'yolo' ? 'bg-[#00ffcc]/20 text-[#00ffcc]' : 'text-gray-400 hover:text-white'
                      }`}
                  >
                    <Crosshair size={11} />
                    Live YOLO Detection
                  ...
                  <button
                    ...
                    className={`px-3 py-1 flex items-center gap-1.5 transition-colors ${playerMode === 'raw' ? 'bg-[#00ffcc]/20 text-[#00ffcc]' : 'text-gray-400 hover:text-white'
                      }`}
                  >
                    <Film size={11} />
                    HTML5 Player
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {availableVideos.map((v) => (
                  <button
                    ...
                    className={`text-xs px-2.5 py-1 rounded-lg border font-mono transition-all ${source === v.path
                ...
                <button
                  ...
                  className={`text-xs px-2.5 py-1 rounded-lg border font-mono transition-all ${source === 'synthetic'
                      ? 'border-[#00ffcc] bg-[#00ffcc]/15 text-[#00ffcc]'
                      : 'border-white/10 bg-white/5 text-gray-400 hover:border-white/20 hover:text-white'
                    }`}
                >
