      ...
      <div className="flex gap-2 mb-2 overflow-x-auto pb-1 scrollbar-hide text-xs">
        {['ALL', 'CRITICAL', 'OBSTACLE', 'CLOUDINARY', 'TELEMETRY'].map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-2 py-1 rounded border whitespace-nowrap ${filter === f ? 'bg-[#00ffcc] text-black border-[#00ffcc] font-bold' : 'bg-transparent text-gray-400 border-gray-600 hover:border-gray-400'
              }`}
          >
            {f}
          </button>
        ...
        <AnimatePresence>
          {filteredAlerts.map((alert) => (
            ...
            >
              ...
              <span
                className={`font-semibold shrink-0 select-none mr-2 ${alert.type === 'critical' ? 'text-red-500' : alert.type === 'warning' ? 'text-yellow-400' : 'text-[#00ffcc]'
                  }`}
              >
